...
function HowItWorksCard({ index, data }: HIWCProps) {
	return (
		<div className={`relative ${(index + 1) % 2 === 0 ? "top-14 md:top-20" : ""}`}>
			<div className={`bg-navy rounded-xl py-4 md:py-8 px-2 md:px-6 h-60 md:h-80`}>
				...
				<div className="bg-primary rounded-full flex items-center text-darkGrey p-2 mt-3 mb-6 w-min whitespace-nowrap space-x-2">
					{data.images.map((img, index) => (
						<div key={index} className="items-center w-4 h-4 md:w-8 md:h-8">
